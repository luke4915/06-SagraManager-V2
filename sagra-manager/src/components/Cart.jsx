...
const CartItemModal = ({ item, onClose, onAdd, onRemove, onDelete, onNoteChange }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4"
      ...
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" />
      <div
        className="relative w-full max-w-sm bg-[var(--bg-card)] rounded-2xl border border-[var(--border)] shadow-2xl overflow-hidden"
      ...
      >
        ...
        <div className="flex items-start justify-between px-5 pt-5 pb-4 border-b border-[var(--border)]">
          <div className="flex-1 min-w-0 pr-3">
            ...
            <p className="text-orange-500 font-black text-sm tabular-nums mt-0.5">{(item.price * item.quantity).toFixed(2)} €</p>
          </div>
          <button onClick={handleClose}
            className="p-2 rounded-xl bg-[var(--bg-card-2)] border border-[var(--border)] text-[var(--text-muted)] hover:text-[var(--text-main)] transition-colors shrink-0">
            <X size={15} />
          </button>
        </div>
        ...
        <div className="px-5 py-4 border-b border-[var(--border)]">
          ...
          <div className="flex items-center justify-between gap-3">
            <button onClick={() => onRemove(item)}
              className="flex-1 py-3 rounded-xl bg-[var(--bg-card-2)] border border-[var(--border)] flex justify-center text-[var(--text-main)] hover:border-orange-500 hover:text-orange-500 transition-all active:scale-95">
              <Minus size={16} />
            </button>
        ...
        <div className="px-5 py-4 border-b border-[var(--border)]">
          ...
          <textarea
            ...
            className="w-full bg-[var(--bg-input)] px-3 py-2.5 rounded-xl text-sm font-medium outline-none ring-1 ring-[var(--border)] focus:ring-orange-500 text-[var(--text-main)] resize-none placeholder:text-[var(--text-muted)] placeholder:font-normal"
          />
        </div>
...
const Cart = ({
  ...
  return (
    <>
      <div className="flex flex-col h-full bg-[var(--bg-card)] rounded-2xl border border-[var(--border)] overflow-hidden">
          ...
          {mergedCart.length === 0 ? (
            ...
            mergedCart.map(item => (
              <div key={cartKey(item)}
                ...
                className="px-3 py-2.5 rounded-xl cursor-pointer border border-transparent bg-[var(--bg-card-2)] hover:border-[var(--border)] active:scale-[0.99] transition-all">
        ...
        <div className="px-4 py-4 border-t border-[var(--border)] space-y-3 bg-[var(--bg-card-2)]">
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-[var(--bg-card)] px-3 py-2 rounded-xl border border-[var(--border)]">
              ...
              <input type="text" value={amountReceived} onChange={e => setAmountReceived(e.target.value)}
                placeholder="0.00"
